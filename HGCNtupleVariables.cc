...
std::pair<float, float>  HGCNtupleVariables::getNoise(std::pair<int,std::pair<int,int>>layer_chip_chan) {
  std::pair<float, float>  temp_noise = {0,0};
  std::map<std::pair<int,std::pair<int,int>>, std::pair<float, float> >::iterator it = noise_map.find(layer_chip_chan);
  if(it != noise_map.end()) {
    temp_noise = it->second;
  }
  else {
    int     layer_ = layer_chip_chan.first;
    std::pair<int,int> chip_chan = layer_chip_chan.second;
    std::cout<<"Value NOT found for Module = "<<layer_<<" & chip = "<<chip_chan.first<<"\t"<<chip_chan.second<<std::endl;
  }
  return temp_noise;
}
